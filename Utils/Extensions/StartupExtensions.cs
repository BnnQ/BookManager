...
using BookManager.Services.MapperProfiles;
using Microsoft.Azure.Cosmos;
using Microsoft.Azure.Cosmos.Fluent;

namespace BookManager.Utils.Extensions;
...
public static class StartupExtensions
{
    public static WebApplicationBuilder ConfigureServices(this WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddTransient<IRepository<Book, string>, CosmosBookRepository>();

        builder.Services.AddAutoMapper(profiles => { profiles.AddProfile<BookMapperProfile>(); });

        builder.Services.AddCors(options =>
        {
            options.AddDefaultPolicy(policyBuilder =>
            {
                policyBuilder//.WithOrigins("https://localhost:44476")
                    .AllowAnyOrigin()
                    .AllowAnyMethod()
                    .AllowAnyHeader();
                //.AllowCredentials();
